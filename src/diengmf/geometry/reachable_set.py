...
def reachable_set(
    ...
    thrust_ensemble = simulate_thrust(
        thrust_key, ensemble, num_simulations, delta_v_magnitude
    )
    thrust_ensemble = eqx.filter_vmap(dynamical_system.flow)(
        0.0, time_horrizon, thrust_ensemble
    )
...
@eqx.filter_jit
def angular_convex_hull(
    ensemble_angles: Float[Array, "n_samples 2"], n_directions=100
) -> Float[Array, "n_hull_vertices 2"]:
    azimuth_range = jnp.max(ensemble_angles[:, 0]) - jnp.min(ensemble_angles[:, 0])

    # Handle azimuth wraparound
    wrapped_angles = jnp.where(
        azimuth_range > jnp.pi,
        jnp.where(
            ensemble_angles[:, 0] < 0,
...
def fan_triangulate(hull: ptx.ConvexHull) -> Float[Array, "n_triangles 3 2"]:
    ...
    triangles = triangles.at[:, 2, :].set(
        jnp.roll(hull_vertices, -1, axis=0)
    )  # Next vertex
...
def barycentric_subdivision(
    triangles: Float[Array, "n_triangles 3 2"],
) -> Float[Array, "4*n_triangles 3 2"]:
