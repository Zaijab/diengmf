...
from diengmf.statistics import EnsembleGaussian
...
class EnKF(AbstractFilter, strict=True):
    ...
    @jaxtyped(typechecker=typechecker)
    @eqx.filter_jit
    def initialize(self, key: Key[Array, ""], belief: AbstractDistribution) -> EnsembleGaussian:
        keys = jax.random.split(key, self.ensemble_size)
        samples = eqx.filter_vmap(belief.sample)(keys)
        return EnsembleGaussian(samples)
    
    @jaxtyped(typechecker=typechecker)
    @eqx.filter_jit
    def predict(
        self,
        posterior_belief: EnsembleGaussian,
        ...
        return EnsembleGaussian(predicted_means)
    ...
    def update_point(
        ...
        if self.debug:
            assert isinstance(
                innovation_cov, Float[Array, "measurement_dim measurement_dim"]
            )
        ...
        if self.debug:
            assert isinstance(posterior_covariance, Float[Array, "state_dim state_dim"])
    ...
    def update(
        ...
        ensemble_covariance = (ensemble_covariance + ensemble_covariance.T) / 2  # Symmetrize
        ensemble_covariance = ensemble_covariance + 1e-12 * jnp.eye(ensemble_covariance.shape[0])
        ...
        return EnsembleGaussian(updated_ensemble)
