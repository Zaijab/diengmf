...
class NormalizingFlow(eqx.Module):
        ...
        *,
        ...
        def make_coupling(k):
            ...
            bijector = RQSBijector(
                input_dim=input_dim,
                num_bins=num_bins,
                range_min=rqs_range_min,
                range_max=rqs_range_max,
        ...
        def make_plu(k):
            return PLULinear(
                input_dim, True, initialization_scale, key=k
            )  # Hard coding use Bias
        ...
        if mask_strategy == "alternating":
            ...
            self.masks = jnp.array(
                [
                    mask_pattern if i % 2 == 0 else 1.0 - mask_pattern
    ...
    def forward(self, x: Array) -> tuple[Array, Array]:
        ...
        dynamic_coupling, static_coupling = eqx.partition(
            self.coupling_layers, eqx.is_array
        )
        ...
        (x_final, total_logdet), _ = jax.lax.scan(
            forward_step, (x, jnp.zeros(())), layer_data
        )
    ...
    def inverse(self, z: Array) -> tuple[Array, Array]:
        ...
        (z_final, total_logdet), _ = jax.lax.scan(
            inverse_step, (z, jnp.zeros(())), layer_data
        )
