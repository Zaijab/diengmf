...
def _normalize_bin_sizes(
    ...
    assert (
        num_bins * min_bin_size <= total_size
    ), f"num_bins * min_bin_size > total_size"
    bin_sizes = jax.nn.softmax(unnormalized_bin_sizes, axis=-1)  # Distrax line 33
    return (
        bin_sizes * (total_size - num_bins * min_bin_size) + min_bin_size
    )  # Distrax line 34
...
def _normalize_knot_slopes(
    ...
    offset = jnp.log(
        jnp.exp(1.0 - min_knot_slope) - 1.0
    )  # Distrax line 45: offset for slope=1 when input=0
    return (
        jax.nn.softplus(unnormalized_knot_slopes + offset) + min_knot_slope
    )  # Distrax line 46
...
def _safe_quadratic_root(a: Array, b: Array, c: Array) -> Array:
    ...
    safe_sqrt = jnp.sqrt(
        jnp.clip(sqrt_diff, jnp.finfo(sqrt_diff.dtype).tiny)
    )  # Distrax line 129
    safe_sqrt = jnp.where(sqrt_diff > 0.0, safe_sqrt, 0.0)  # Distrax line 131
...
def _rational_quadratic_spline_fwd(
    ...
    z = jnp.clip(z, 0.0, 1.0)  # Distrax line 81: Ensure z in [0,1]
    sq_z = z * z
...
def _rational_quadratic_spline_inv(
    ...
    w = jnp.clip(w, 0.0, 1.0)  # Distrax line 173: Ensure w in [0,1]
    ...
    z = jnp.clip(z, 0.0, 1.0)  # Distrax line 182: Ensure z in [0,1]
...
class RQSBijector(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        assert min_bin_size > 0.0, f"min_bin_size <= 0"
        assert min_knot_slope > 0.0, f"min_knot_slope <= 0"
    ...
    def _get_spline_params(self, params: Array) -> tuple[Array, Array, Array]:
        ...
        assert knot_slopes.shape == (
            self.num_bins + 1,
        ), f"knot_slopes shape {knot_slopes.shape}"
    ...
    def forward(
        ...
        def forward_scalar(
            x_scalar: Array, params_scalar: Array
        ) -> tuple[Array, Array]:
    ...
    def inverse(
        ...
        def inverse_scalar(
            y_scalar: Array, params_scalar: Array
        ) -> tuple[Array, Array]:
            x_pos, y_pos, knot_slopes = self._get_spline_params(params_scalar)
