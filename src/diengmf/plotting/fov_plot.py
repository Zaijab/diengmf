def plot_tracking_fov(
    predicted_state,
    true_state,
    step,
    elevation_fov=(5 * jnp.pi / 180),
    azimuth_fov=(5 * jnp.pi / 180),
    range_fov=1.0,
):
    ...
    is_measurable = tracking_measurability(
        true_state, predicted_state, elevation_fov, azimuth_fov, range_fov
    )
    ...
    ax.scatter(
        *true_state[:3],
        c=true_color,
        s=100,
        label=f'True State ({"Measurable" if is_measurable else "Not Measurable"})',
    )

    # Dynamic axis limits to show both states and wireframe
    all_points = np.vstack(
        [corners, predicted_state[:3].reshape(1, -1), true_state[:3].reshape(1, -1)]
    )
    margin = 0.02
    ...
    ax.set_zlabel("Z")
    ax.legend()
    plt.title(f"Optical Sensor FOV - Step {step}")
