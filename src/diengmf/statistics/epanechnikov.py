...
def pdf_epanechnikov(
    ...
    density = (
        ...
        / (
            2
            * ((jnp.pi ** (n / 2)) / (gamma((n / 2) + 1)))
            * ((n + 4) ** ((n + 2) / 2))
        )
    ) * ((n + 4) - (standard_input @ standard_input))

    density = jnp.where(
        standard_input @ standard_input <= (n + 4), normalizing_constant * density, 0.0
    )
    return density
...
def sample_epanechnikov(
    key: Key[Array, "1"], mu, sigma, num_samples, standard=False, debug=False
):
    n = mu.shape[0]
    key, subkey1, subkey2 = jax.random.split(key, 3)
    s = jax.random.multivariate_normal(
        subkey1, jnp.zeros(n), jnp.eye(n), (num_samples,)
    )
