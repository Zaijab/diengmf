...
def find_random_second_sensor_state(
    ...
    elevation_constraint = jnp.where(
        side == 0,
        -constraint_distance + 2 * constraint_distance * local_t,
        jnp.where(
            side == 1,
    ...
    points_3d = jnp.stack(
        [
            jnp.cos(el_rad) * jnp.cos(az_rad),
            jnp.cos(el_rad) * jnp.sin(az_rad),
            jnp.sin(el_rad),
        ],
        axis=1,
    )
...
def place_sensors(
    key: jax.Array,
    candidates: Float[Array, "n_candidates 2"],
    n_sensors: int,
    ...
    final_positions, _, _ = jax.lax.fori_loop(
        0, n_sensors, place_single_sensor, (sensor_positions, placement_mask, key)
    )
