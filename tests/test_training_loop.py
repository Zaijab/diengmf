...
from diengmf.models import (MaskedCoupling, NormalizingFlow, PLULinear,
                            RQSBijector)
...
def compute_debug_metrics(
    model: eqx.Module, system: eqx.Module, debug_key: Array
) -> tuple[Float[Array, ""], Float[Array, "1000 3"]]:
    gen_key, attractor_key, sample_key = jax.random.split(debug_key, 3)
    base_plot_samples = jax.random.multivariate_normal(
        sample_key, jnp.zeros(system.dimension), jnp.eye(system.dimension), (1000,)
    )
    plot_samples, _ = eqx.filter_vmap(model.forward)(base_plot_samples)
    plot_data = jnp.concatenate(
        [
            plot_samples[:, : min(3, system.dimension)],
            jnp.zeros((1000, max(0, 3 - system.dimension))),
        ],
        axis=1,
    )
    return plot_data
...
def plot_callback(step, score, samples, system_name, dim, dynamical_system):
    ...
    if hasattr(ax, "set_zlim"):
        ax.set_zlim(dynamical_system.plot_limits[2])
    plt.title(f"Step {step}: {system_name} - Score: {score:.3f}")
    plt.show()
...
def debug_with_plots(carry_data):
    ...
    jax.debug.callback(
        plot_callback, step, 0, samples, type(system).__name__, system.dimension, system
    )
    return None
...
def io_operations_callback(step, loss, samples, system_name, dim, dynamical_system):
    ...
    if hasattr(ax, "set_zlim"):
        ax.set_zlim(dynamical_system.plot_limits[2])
    plt.title(f"Step {step}: {system_name} - Loss: {loss:.3f}")
    plt.show()
...
@eqx.filter_jit
def training_loop(
    key: Array, model: eqx.Module, system: eqx.Module, optim: optax.Schedule
):
    ...
    (final_batch, final_model_arrays, final_opt_arrays, _), losses = jax.lax.scan(
        scan_step, initial_carry, xs=jnp.zeros(20_001)
    )
        ...
        optim = optax.chain(
            ...
            optax.adamw(
                learning_rate=optax.cosine_decay_schedule(
                    init_value=3e-4, decay_steps=5000, alpha=0.03
                ),
                weight_decay=1e-5,
            ),
        )

        final_model, final_opt_state = training_loop(
            key, model, dynamical_system, optim
        )
