...
def test_rqs_hyperparameter_invertibility(
    ...
    rqs = RQSBijector(input_dim=system.dimension, key=key, **config)
...
@pytest.mark.parametrize(
    "config", RQS_HYPERPARAMETER_GRID[:5]
)  # Test subset for training
@pytest.mark.parametrize(
    "system_name,system_factory", DYNAMICAL_SYSTEMS[:2]
)  # Faster systems
def test_rqs_hyperparameter_training_stability(
    config, system_name, system_factory, test_keys
):
    ...
    ), f"Training broke invertibility for {system_name}"
...
def test_rqs_parameter_shapes_across_configs():
    ...
    for i, config in enumerate(RQS_HYPERPARAMETER_GRID):
        ...
        rqs = RQSBijector(input_dim=2, key=test_key, **config)  # Fixed for this test
...
def suggest_rqs_config(trial):
    ...
    return {
        ...
        "param_init": trial.suggest_categorical(
            "param_init",
            [
