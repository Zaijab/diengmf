...
class RQSBijector(eqx.Module):
    ...
    def _normalize_params(self, params: Array):
        ...
        widths_unnorm, heights_unnorm, slopes_unnorm = (
            params[:K],
            params[K : 2 * K],
            params[2 * K :],
        )
        ...
        heights = (
            heights_norm * (total_size - K * self.min_bin_size) + self.min_bin_size
        )
        ...
        y_pos = (
            jnp.concatenate([jnp.array([0.0]), jnp.cumsum(heights)]) + self.range_min
        )
    ...
    def forward_with_params(self, x: Array, params: Array) -> tuple[Array, Array]:
        ...
        logdet_linear_left, logdet_linear_right = jnp.log(slope_left), jnp.log(
            slope_right
        )
        ...
        num_derivative = (
            s * s * (slope_k1 * xi * xi + 2.0 * s * xi_xi_1m + slope_k * xi_1m * xi_1m)
        )
        ...
        # Combine regions
        y = jnp.where(
            below_range, y_linear_left, jnp.where(above_range, y_linear_right, y_spline)
        )
        logdet = jnp.where(
            below_range,
            logdet_linear_left,
...
class MaskedCoupling(eqx.Module):
    ...
    def __init__(
        ...
        input_dim: int,
        bijector: RQSBijector,
        mask_type: Literal["half", "alternating"] = "half",
    ...
    def forward(self, x: Array) -> tuple[Array, Array]:
        ...
        for i in range(transform_dim):
            ...
            y_vals, logdets = eqx.filter_vmap(apply_bijector_to_batch)(
                x_transform[:, i], params_reshaped[:, i, :]
            )
...
class RQSINNConfig:
    ...
    def get_mask_sequence(self) -> List[Literal["half", "alternating"]]:
        ...
        if self.auto_reverse_masks:
            return [
                "half" if i % 2 == 0 else "alternating"
...
class RQSINN(eqx.Module):
    ...
    def __init__(self, config: RQSINNConfig, *, key: Array):
        ...
        bijector = RQSBijector(
            range_min=config.spline_range_min, range_max=config.spline_range_max
        )
...
def test_rqs_inn():
    ...
    print(
        f"Forward: x.shape={x_test.shape} -> z.shape={z.shape}, logdet.shape={fwd_logdet.shape}"
    )

    # Inverse pass
    x_recon, inv_logdet = model.inverse(z)
    print(
        f"Inverse: z.shape={z.shape} -> x_recon.shape={x_recon.shape}, logdet.shape={inv_logdet.shape}"
    )
    ...
    ), f"Poor reconstruction: {reconstruction_error:.2e}"
    assert (
        logdet_consistency < 1e-8
    ), f"Poor logdet consistency: {logdet_consistency:.2e}"

    print("✅ RQS INN test passed!")

    # Count parameters
    param_count = sum(
        jnp.size(leaf) for leaf in jax.tree_leaves(eqx.filter(model, eqx.is_array))
    )
