...
def test_manual_composition_equivalence():
    ...
    manual_couplings = [
        MaskedCoupling(input_dim, bijector, key=keys[i]) for i in range(num_layers)
    ]
    manual_plus = [
        PLULinear(input_dim, True, key=keys[num_layers + i]) for i in range(num_layers)
    ]
    ...
    for coupling, plu in zip(manual_couplings, manual_plus):
        manual_x, ldj = coupling.forward(manual_x)
        manual_logdet += ldj
        manual_x, ldj = plu.forward(manual_x)
        manual_logdet += ldj
...
def test_parametric_invertibility(input_dim, num_layers):
    ...
    assert (
        logdet_error < 1e-12
    ), f"Dim {input_dim}, Layers {num_layers}: logdet error {logdet_error}"
