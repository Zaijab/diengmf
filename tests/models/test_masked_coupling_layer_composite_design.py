...
from diengmf.models import MaskedCoupling, RQSBijector
...
def test_distrax_comparison():
    ...
    try:
        ...
        for x in test_values:
            ...
            our_x_recon, our_inv_logdet = our_rqs.inverse_with_params(
                our_y, test_params
            )

            # Distrax implementation
            distrax_y, distrax_logdet = distrax_rqs.forward_and_log_det(x)
            distrax_x_recon, distrax_inv_logdet = distrax_rqs.inverse_and_log_det(
                distrax_y
            )
            ...
            assert (
                abs(x - our_x_recon) < 1e-10
            ), f"Our RQS invertibility failed at x={x}"
            assert (
                abs(x - distrax_x_recon) < 1e-10
            ), f"Distrax RQS invertibility failed at x={x}"

        print(
            f"✓ Max RQS differences: y={max_y_diff:.2e}, logdet={max_logdet_diff:.2e}"
        )
        ...
        def simple_conditioner(x):
            ...
            return jnp.concatenate(
                [conditioning_sum, conditioning_sum], axis=-1
            )  # [scale, shift]
        ...
        distrax_mask = jnp.array(
            [True, False]
        )  # First dim unchanged, second transformed
        distrax_coupling = distrax.MaskedCoupling(
            mask=distrax_mask, conditioner=simple_conditioner, bijector=simple_bijector
        )
        ...
        distrax_x_recon, distrax_inv_logdet = distrax_coupling.inverse_and_log_det(
            distrax_y
        )
        ...
        print(
            f"  Logdet consistency: {abs(distrax_logdet + distrax_inv_logdet) < 1e-10}"
        )

        # Verify our coupling follows same pattern
        our_coupling = MaskedCoupling(
            input_dim=2, bijector=RQSBijector(), debug=False, key=key
        )
...
def run_all_tests():
    ...
    test_vals = jnp.concatenate(
        [
            ...
            jnp.array(
                [-6.0, -5.1, -4.9, 4.9, 5.1, 6.0, 0.0, 1e-8, -1e-8, 100.0]
            ),  # Edge cases
        ]
    )
    ...
    for i, x in enumerate(test_vals):
        ...
        if i < 5:
            print(
                f"  x={x:.3f} -> y={y:.3f} -> x_recon={x_recon:.3f}, error={fwd_inv_error:.2e}"
            )

    print(f"✓ Max forward-inverse error: {max_fwd_inv_error:.2e}")
    print(f"✓ Max logdet error: {max_logdet_error:.2e}")
    assert (
        max_fwd_inv_error < 1e-10
    ), f"RQS invertibility failed: {max_fwd_inv_error:.2e}"
    ...
    for i, x_pair in enumerate(x_vals):
        ...
        if i < 5:
            print(
                f"  x={x_pair} -> y={y_pair[0]} -> x_recon={x_recon[0]}, error={coupling_error:.2e}"
            )
    ...
    assert (
        max_coupling_error < 1e-10
    ), f"Coupling invertibility failed: {max_coupling_error:.2e}"
    assert (
        max_coupling_logdet_error < 1e-10
    ), f"Coupling logdet failed: {max_coupling_logdet_error:.2e}"
    ...
    slope_right = (
        jax.nn.softplus(params[2 * K + K] + slopes_offset) + 1e-4
    )  # Last slope
    ...
    print(
        f"  x={x_outside} (outside range) -> y={y_linear:.6f} (expected {expected_y:.6f})"
    )
    ...
    assert (
        linear_logdet_error < 1e-10
    ), f"Linear case logdet failed: {linear_logdet_error:.2e}"
