...
class FlowHyperparameters:
    ...
    layer_pattern: Literal["coupling_only", "sandwich", "interleaved", "multiscale"] = (
        "sandwich"
    )
    ...
    lr_schedule: Literal["constant", "cosine", "exponential", "warmup_cosine"] = (
        "cosine"
    )
...
def get_search_space_for_optuna():
    ...
    return {
        ...
        # Regularization
        "dropout_rate": {"type": "float", "low": 0.0, "high": 0.3, "importance": "low"},
        "weight_decay": {
            "type": "loguniform",
            "low": 1e-6,
            "high": 1e-3,
...
def estimate_model_complexity(config: FlowHyperparameters, input_dim: int):
    ...
    if config.transform_type in ["rqs", "mixed"]:
        ...
        conditioner_params = (
            ...
            + config.conditioner_hidden_dim**2
            * (config.conditioner_depth - 2)  # Hidden layers
            + config.conditioner_hidden_dim
            * (input_dim // 2)
            * (3 * config.num_bins + 1)  # Output layer
        )
